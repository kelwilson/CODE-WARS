...
function validateSudoku(board) {
    const createEmptyArrays = n => Array.from({ length: n }, () => []);
    
    const rows = board,
          cols = createEmptyArrays(9),
          blocks = createEmptyArrays(9);
    
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        cols[col].push(board[row][col]);
        
        const curBlock = Math.floor(row / 3) * 3 + Math.floor(col / 3);
        blocks[curBlock].push(board[row][col])
      }
    }
    
    for (const numbersSet of [...rows, ...cols, ...blocks]) {
      if (numbersSet.includes(0) || new Set(numbersSet).size < 9) return false;
    }
