...
function flip(stack) {
    ...
    // create function to flip stack 
    //  sort fliped array
    // check for the position of the largest 
    // return array
      let sequence = [];
  
    // Helper function to flip the stack up to the given index
    function flipAt(index) {
      let flipped = stack.slice(0, index + 1).reverse(); // Flip part of the stack
      stack = flipped.concat(stack.slice(index + 1)); // Combine the flipped part with the remaining part
      sequence.push(index); // Record the flip position
    }
  
    // Start sorting by bringing the largest to the top and flipping it to its position
    for (let i = stack.length - 1; i > 0; i--) {
      // Find the index of the largest unsorted element in the stack
      let maxIndex = stack.indexOf(Math.max(...stack.slice(0, i + 1)));
  
      // If the largest is not already at its correct position
      if (maxIndex !== i) {
        // If the largest is not already at the top, flip it to the top
        if (maxIndex !== 0) {
          flipAt(maxIndex); // Flip the largest element to the top
        }
